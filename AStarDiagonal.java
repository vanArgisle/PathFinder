...
public class AStarDiagonal {
	...
	public AStarDiagonal(Manager iface) {
	...
	private void setHeuristics(int x, int y, int hType) {
		// inter.setHeuristic(points.get(0), points.get(1), 0, hType);
		int length = inter.getHeuristic(x, y, hType);

		if (x != 0 && y != 0) {
			checkHeuristic(x - 1, y - 1, length + 15, hType);
		}
		if (x != 0) {
			checkHeuristic(x - 1, y, length + 10, hType);
		}
		...
		if (y != inter.iface.getGridSizeY() - 1) {
			checkHeuristic(x, y + 1, length + 10, hType);
		}

		if (x != inter.iface.getGridSizeX() - 1 && y != inter.iface.getGridSizeY() - 1) {
			checkHeuristic(x + 1, y + 1, length + 15, hType);
		}
	...
	private void runAStar(int x, int y) {
		...
		if (x != 0 && y != 0) {
			checkSide(x - 1, y - 1, "UpLeft", cLength + 14);
		}
		if (x != 0) {
			checkSide(x - 1, y, "Up", cLength + 10);
		}
		if (x != 0 && y != inter.iface.getGridSizeY() - 1) {
			checkSide(x - 1, y + 1, "UpRight", cLength + 14);
		}
		if (y != inter.iface.getGridSizeY() - 1) {
			checkSide(x, y + 1, "Right", cLength + 10);
		}
		if (x != inter.iface.getGridSizeX() - 1 && y != inter.iface.getGridSizeY() - 1) {
			checkSide(x + 1, y + 1, "DoRight", cLength + 14);
		}

		if (x != inter.iface.getGridSizeX() - 1) {
			checkSide(x + 1, y, "Down", cLength + 10);
		}
		if (x != inter.iface.getGridSizeX() - 1 && y != 0) {
			checkSide(x + 1, y - 1, "DoLeft", cLength + 14);
		}

		if (y != 0) {
			checkSide(x, y - 1, "Left", cLength + 10);
		}

		System.out.println("ran A Star Diagonal");
	}
